...
| 点击分割线并拖动 | 调整相邻 pane 的宽高比 |
| `Ctrl + 拖拽` pane | 把该 pane 从平铺格中拖出为自由窗口 |
| 拖到另一个 pane 上释放 | 往回折进该 pane（合并为相邻分割） |
| 拖到空白处释放 | 保持为自由窗口，可再从标题栏拖拽/缩放 |

> 平铺布局是**房间内共享**的：窗格划分、分割比例、`Ctrl + 拖拽` 派生的自由等操作都会通过 Yjs 同步给所有协作用户，任一成员调整布局，其余成员实时看到相同窗格。**自由 / 平铺的切换是本地的查看偏好**，不随共享布局切换，并保存在浏览器中以在刷新后恢复；自由窗口的 position/size 仍由服务端共享几何同步。
