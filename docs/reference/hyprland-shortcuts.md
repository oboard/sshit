...
本页汇总 Hyprland 官方 Wiki 中常见的默认绑定，便于在 `sshit` 工作区中使用 Hyprland 时快速查阅。实际可用按键取决于你的 Hyprland 配置；如果覆盖了默认绑定，请以本机配置为准。
...
- **平铺**：将当前窗口自动排列为网格，以更接近 Hyprland 的平铺工作流；切回浮动会恢复切换前的位置和尺寸。
